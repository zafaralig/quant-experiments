...
df.loc['12-July-2022',['Open', 'High', 'Low', 'Close']].plot(grid=True,linewidth=1,figsize=(14,9))
...
df['position'] = np.where((df['sma']<df['Close']),1,0)
...
df['cc_returns'] = df['cc_returns'] + 1  #cumprod() of df['cc_returns'] is equity curve

df[['cc_returns', 'strategy_returns']].cumprod().plot(grid=True,linewidth=1,figsize=(14,9)) # equity curve plot of cc_returns and strategy_returns
